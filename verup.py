...
def version_up(filefullname: str):
    ...
    if len(filelist) != 0:
        filelist = sorted(filelist, key=lambda x: int(x.split("_")[-1].split(".")[0][1:]) if x.split("_")[-1].split(".")[0][1:].isnumeric() else -1, reverse=True)
    ...
    print(f"{filefullname} is copied {newfilename}")
